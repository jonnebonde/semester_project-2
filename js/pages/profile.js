...
async function getMultipleProfileInfo() {
  try {
    const profileBidsOnListings = await getProfileInfo(
      "/bids?_listings=true&_bids=true&_active=true",
    );
    const profileListings = await getProfileInfo(
      "/listings?_bids=true&_active=true",
    );
